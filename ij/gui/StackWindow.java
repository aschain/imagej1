...
public class StackWindow extends ImageWindow implements Runnable, AdjustmentListener, ActionListener, MouseWheelListener {
	...
	void addScrollbars(ImagePlus imp) {
		...
		if (nChannels>1) {
			...
			add(cSelector);
			win.sliderHeight+=cSelector.getPreferredSize().height+ImageWindow.VGAP;
		...
		if (nSlices>1) {
			...
			add(zSelector);
			win.sliderHeight+=zSelector.getPreferredSize().height+ImageWindow.VGAP;
		...
		if (nFrames>1) {
			...
			add(tSelector);
			win.sliderHeight+=tSelector.getPreferredSize().height+ImageWindow.VGAP;
